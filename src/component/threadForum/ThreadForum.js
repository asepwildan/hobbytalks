...
export default function ThreadForum({ shorting, kategoriValue }) {
    ...
    const [cobaState, setCobaState] = useState(false);
    const dispatch = useDispatch();
    const { threadList, totalPage, nextPage, currentPage, loading, error } = useSelector(
    ...
    //         setPage("1");
    //     } 
    //     console.log(currentPage, "test curentPage");
    // }, [currentPage]);
    ...
    return (
        <div className={styles.threadContainer}>
                ...
                <>
                    ...
                    <div key={thread.userId} className={styles.threadListProfile}>
                        <div className={styles.titleThreadProfile}>
                                ...
                                <div>
                                    ...
                                    <div className={styles.threadProfileInfo}>
                                        {thread.userId === null ? (
                                            ...
                                            <Link
                                                to={
                                                    thread.userId._id === profileInfo._id
                                                        ? `/profile`
                                                        : thread.userId._id === null
                                                        ? `/forum`
                                                        : `/account-detail/?xyz=${thread.userId._id}`
                                                }
                                            >
                                                {" "}
                                                <Avatar
                            ...
                            <div className={styles.threadActionanddate}>
                                <Link
                                    to={`/thread-detail/?xyz=${thread._id}`}
                                    style={{ textDecoration: "none" }}
                                >
                                ...
                                <p className={styles.dateThread}>{thread.date.slice(0, 10)}</p>
                            </div>
                        </div>
                    </div>
                </>
            ...
            {paginationCon === "home" || paginationCon === "" ? (
                ...
                <div className={styles.paginationContainer}>
                    <Pagination count={totalPage} page={currentPage} onChange={handleChangeFollow} />
                </div>
            ) : (
                <div className={styles.paginationContainer}>
                    <Pagination count={totalPage} page={currentPage} onChange={handleChangeCat} />
                </div>
            )}
        </div>
    );
