...
import NotfoundTopic from "./img/notfound.png";
export default function Likerekomendasi() {
    ...
    useEffect(() => {
        axios
            .get("https://hobbytalk-be-glints.herokuapp.com/api/v1/threads/mightlike", {
                headers: {
        ...
        <div className={styles.likerekomendasicontainer}>
            ...
            {!likerekomendasi.length ? (
                <div className={styles.containerNotFound}>
                    ...
                    <p className={styles.notfoundTexth2}>Oops We didn't find a topic for you</p>
                    <p className={styles.notfoundTextDirectInfo}>
                        why don't you choose a category that suits you
                    </p>
                    <Link to={"/category"} style={{ textDecoration: "none" }}>
                        <button className={styles.directButtonTopic}>Browse Topic</button>
                    </Link>
                </div>
            ) : (
                likerekomendasi.map((categoryLike) => (
                    <div>
                        <div className={styles.threadInfoLikeContainer}>
                            <div className={styles.titleThreadLike}>
                                <Link
                                    to={`/thread-detail/?xyz=${categoryLike._id}`}
                                    style={{ textDecoration: "none" }}>
                                    <p className={styles.titleInfoText}>{categoryLike.title}</p>
                                </Link>
                                <div className={styles.threadRekomAkun}>
                                    <Avatar
                                        className={styles.avatarAkun}
                                        src={categoryLike.userId.avatar}
                                        alt="img1"
                                    />
                                    <p className={styles.threadAccountName}>
                                        {categoryLike.userId.name}
                                    </p>
                                </div>
                                <div className={styles.threadActionContainer}>
                                    <div className={styles.threadAction}>
                                        ...
                                        <div className={styles.threadVoteInfo}>
                                            {categoryLike.likeCount}
                                        </div>
                                    </div>
                                    <div className={styles.threadAction}>
                                        <svg
